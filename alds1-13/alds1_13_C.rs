...
fn main() {
    ...
    let mut l = 0;
    loop {
        let result = dfs(&mut data, x, y, 0, l, None);
        if result != -1 {
            println!("{}", result);
            break;
        }
...
fn dfs(data: &mut [[i32; NUM]; NUM], x: usize, y: usize, d: i32, l: i32, prev: Option<usize>) -> i32 {
    let h = cal(data);
    if h == 0 {
        return d;
    }
    ...
    for dir in 0..4 {
        ...
        data[x][y] = data[nx][ny];
        data[nx][ny] = 0;
        let res = dfs(data, nx, ny, d + 1, l, Some(dir));
        data[nx][ny] = data[x][y];
        data[x][y] = 0;
        if res != -1 {
            return res;
